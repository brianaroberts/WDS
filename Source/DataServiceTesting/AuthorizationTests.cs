...
namespace DataServiceTesting
{
    public class AuthorizationTests
    {
        ...
        public void TestStartup()
        {
            ...
            WDS.Startup startup = new WDS.Startup(config, hosting);
            startup.ConfigureServices(services);
            startup.Configure(application, hosting);
            
        }


        [Test]
        public void TestGetRolesFromAPIKey()
        {
            List<string> result = Roles.GetRolesFromAPIKey(42);
            Assert.That(result, Is.True);
        }

        [Test]
        public void TestWriteRolesToDB()
        {
            bool result = Roles.WriteRolesToDB("Scott", new List<string> { "Admin" });
            Assert.That(result, Is.True);
        }
    }
}